...
def process_excel_file(df: pd.DataFrame) -> openpyxl.Workbook:
    ...
    base_columns = ['Register No', 'Name', 'College ID']
    num_subjects = (num_columns - len(base_columns)) // 4
    
    expected_columns = base_columns + [
    item for i in range(1, num_subjects + 1)
    for item in (f'Subject Code {i}', f'Subject Name {i}', f'Marks {i}', f'Result {i}')
]

    
    if len(expected_columns) > num_columns:
        st.warning("Mismatch detected: Some subjects may be missing. Processing available data.")
        expected_columns = expected_columns[:num_columns]
    elif len(expected_columns) < num_columns:
        st.warning("Mismatch detected: Extra columns found. Processing valid data.")
    ...
    for subject_num in range(num_subjects):
        marks_col = start_col + subject_num * 7 + 2  # Adjust for flexible subject placement
        insert_col = marks_col + 1
        
        sheet.insert_cols(insert_col, 3)
        for idx, col_name in enumerate(['Internal', 'External', 'Total']):
            sheet.cell(row=1, column=insert_col + idx, value=f'{col_name} {subject_num + 1}')
...
def create_department_batches(workbook: openpyxl.Workbook) -> io.BytesIO:
    department_codes = {
        ...
        '31B': 'MSW', '21B': 'Political Science', '31M': 'M. Political Science'
    }
    ...
    for row in range(2, sheet.max_row + 1):
        ...
        if not register_no or len(register_no) < 5:
            continue
        ...
        if dept_code in department_codes:
            ...
            row_values = [cell.value for cell in sheet[row] if cell.value is not None]
